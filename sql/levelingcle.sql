...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : jeu. 27 avr. 2023 à 21:37
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateGamePc` (IN `p_idGame` INT(11), IN `p_gamePCDateSortie` VARCHAR(50), IN `p_gamePcModeleEco` VARCHAR(50), IN `p_gamePcPrix` FLOAT, IN `p_gamePcOs` VARCHAR(50), IN `p_gamePcProc` VARCHAR(50), IN `p_gamePcCg` VARCHAR(50), IN `p_gamePcTaille` FLOAT, IN `p_gameName` VARCHAR(50), IN `p_gameDescription` VARCHAR(1000), IN `p_gameGenre` VARCHAR(100), IN `p_gameAvis` VARCHAR(100), IN `p_gameClassification` VARCHAR(100), IN `p_gameMode` VARCHAR(100), IN `p_gameImg` VARCHAR(100), IN `p_gameTrailer` VARCHAR(100), IN `p_gameImgOther` VARCHAR(100))  Begin
    update tblgamespc set gamePcModeleEco = p_gamePcModeleEco, gamePCDateSortie = p_gamePCDateSortie, gamePcPrix = p_gamePcPrix, gamePcOs=p_gamePcOs, gamePcProc = p_gamePcProc, gamePcCg = p_gamePcCg , gamePcTaille = p_gamePcTaille, gameName = p_gameName, gameDescription = p_gameDescription,
    gameGenre = p_gameGenre ,gameAvis = p_gameAvis, gameClassification =p_gameClassification, gameMode =p_gameMode, gameImg =p_gameImg, gameTrailer = p_gameTrailer, gameImgOther = p_gameImgOther where idGame = p_idGame ; 
...
(33, 'GTA', 'Le monde', 'Multijoeur', '19', '17', 'Multi', 'AppJeuPc', 'AppJeuPc', 'AppJeuPc');
...
CREATE TABLE IF NOT EXISTS `tblgamespc` (
  `idGame` int(11) NOT NULL,
  `gamePCDateSortie` varchar(50) NOT NULL,
...
INSERT INTO `tblgamespc` (`idGame`, `gamePCDateSortie`, `gamePcModeleEco`, `gamePcPrix`, `gamePcOs`, `gamePcProc`, `gamePcCg`, `gamePcTaille`, `gameName`, `gameDescription`, `gameGenre`, `gameAvis`, `gameClassification`, `gameMode`, `gameImg`, `gameTrailer`, `gameImgOther`) VALUES
...
(22, '12/02/2003', 'Payant à l aquisition', 35, 'WINDOW', 'AMD 5300', '2070', 23, 'Far Cry 5', 'Far Cry 5 est un jeu d action / aventure jouable en solo. Bienvenue Ã  Hope County dans le Montana, terre de libertÃ© et de bravoure qui abrite un culte fanatique prÃªchant la fin du monde : Edenâ??s Gate.', 'Infiltration | Action | FPS', '14', '18', 'Jouable en solo | Multi en ligne', 'AppJeuPc', 'AppJeuPc', 'AppJeuPc'),
...
(33, '12/03/2020', 'Payant', 40, 'Linux', 'INTEL I7', '2090', 23, 'GTA', 'Le monde', 'Multijoeur', '19', '17', 'Multi', 'AppJeuPc', 'AppJeuPc', 'AppJeuPc');
